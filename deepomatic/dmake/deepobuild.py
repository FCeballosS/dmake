...
class DockerSerializer(YAML2PipelineSerializer):
    ...
    base_image   = FieldSerializer([DockerBaseSerializer(version = 1), "array"], child = DockerBaseSerializer(version = 2), default = [], post_validation = lambda x: [x] if isinstance(x, DockerBaseSerializer) else x, help_text = "Base (development environment) images.")
...
class ServiceDockerSerializer(YAML2PipelineSerializer):
    ...
    def generate_build_docker(self, commands, path_dir, docker_base, build, config, docker):
        tmp_dir = common.run_shell_command('dmake_make_tmp_dir')
        ...
        docker_base_image = docker.get_docker_base_image(self.base_image_variant)
...
class DeploySerializer(YAML2PipelineSerializer):
    ...
    def generate_build_docker(self, commands, path_dir, docker_base, build, config, docker):
        if config.docker_image.has_value():
            config.docker_image.generate_build_docker(commands, path_dir, docker_base, build, config, docker)
...
class DMakeFile(DMakeFileSerializer):
    ...
    def generate_build_docker(self, commands, service_name):
        service = self._get_service_(service_name)
        tmp_dir = service.deploy.generate_build_docker(commands, self.__path__, self.docker, self.build, service.config, self.docker)
